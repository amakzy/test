...
/* toem_errors.c */
void print_string_to_stderr(char *);
int print_char_to_stderr(char);
int print_char_to_fd(char c , int fd);
int print_string_to_fd(char *string, int fd);
...
char *concatenate_string(char *, char *, int);
...
/*toem_getline.c */
ssize_t line_input(info_t *);
int nextline(info_t *, char **, size_t *);
void ccblocker(int);
...
int read_history(info_ t *inf);
int build_history_list(info_t *inf, char *string, int linecount);
...
int delete_node_at_index(list_t **, unsigned int); //dont forget frm builtin1''//
